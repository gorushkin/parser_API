...
import { Rates, Currency, DBResult, Summary, GroupedSummary, Statement } from './types';
...
export class DB {
  ...
  async init(path: string) {
    ...
    this.exportPath = join(this.dbPath, 'export');
    ...
    const isExportPathExist = await this.checkPath(this.exportPath);
    if (!isPathExist) {
    ...
    if (!isExportPathExist) {
      await this.makeDbFolder(this.exportPath);
    }
  }
  ...
  async updateStatement(data: Transaction[], name: string) {
    try {
      ...
      const dataWithCurencyPromises = data.map(async (item, index, list) => {
        ...
        convertedBalance = getRoundedValue(convertedBalance + convertedAmount);
        ...
        const result = {
          ...
          memo: `${item.amount} ${defaultCurrency} (rate: ${rate}) ${item.description}`,
          convertedBalance,
          date: dayjs(item.transactionDate).format('DD.MM.YYYY'),
        };
  ...
  async exportStatement(name: string) {
    const res = await this.getStatement(name);
    if (!res.ok) throw new DBError(res.error);
    const headers = [
      { name: 'Date', key: 'date' },
      { name: 'Payee', key: 'payeeName' },
      { name: 'Memo', key: 'memo' },
      { name: 'Amount', key: 'convertedAmount' },
    ];
    const headerList = headers.map((item) => item.key);
    const header = headers.map((item) => `"${item.name}"`).join(',');
    const transactions = res.data.transactions
      .map((transaction) =>
        headerList.map((item) => `"${transaction[item as keyof Transaction]}"`).join(',')
      )
      .join('\n');
    const fileContent = header + '\n' + transactions;
    const filePath = path.join(this.exportPath, `${name}.csv`);
